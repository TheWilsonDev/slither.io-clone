...
Snake = function (game, spriteKey, x, y) {
  ...
  this.sectionSpawnSpeed = 0.15; // Lower is faster to catch up, higher is smoother/slower. Adjust as needed.
  this.sectionSpawnActivationDistanceFactor = 0.9; // Activates when segment ahead is this * preferredDistance away
...
Snake.prototype = {
  ...
  update: function () {
    ...
    for (var i = 0; i < this.snakeLength; i++) {
      ...
      var finalTargetPathPosition = this.headPath[index];

      if (currentSection.isSpawning) {
        if (!currentSection.hasActivated) {
          // Stage 1: Stay at spawn point, wait for reveal
          currentSection.body.x = currentSection.spawnWorldPosition.x;
          currentSection.body.y = currentSection.spawnWorldPosition.y;

          if (i > 0) {
            // Must have a preceding segment to measure against
            var previousSegment = this.sections[i - 1];
            var distanceToPrevFromBody = Phaser.Point.distance(previousSegment.body, currentSection.spawnWorldPosition);

            if (distanceToPrevFromBody >= this.preferredDistance * this.sectionSpawnActivationDistanceFactor) {
              currentSection.hasActivated = true;
            }
          } else {
            // This case should ideally not happen for a spawning segment if head is always sections[0]
            // and is never marked isSpawning. If it does, auto-activate.
            currentSection.hasActivated = true;
          }
        }

        // Stage 2: If activated, move to final target position
        // This block can be entered on the same frame hasActivated becomes true.
        if (currentSection.hasActivated) {
          if (!finalTargetPathPosition) {
            // Safety: if target is invalid, hold at spawn position and log error or handle
            currentSection.body.x = currentSection.spawnWorldPosition.x;
            currentSection.body.y = currentSection.spawnWorldPosition.y;
            // console.warn("Spawning section has no finalTargetPathPosition!");
          } else {
            var distanceToFinalTarget = Phaser.Point.distance(currentSection.body, finalTargetPathPosition);

            if (distanceToFinalTarget < 1.0) {
              // Snap if close
              currentSection.body.x = finalTargetPathPosition.x;
              currentSection.body.y = finalTargetPathPosition.y;
              currentSection.isSpawning = false; // Fully spawned
              // Optional: Clean up helper properties
              // delete currentSection.spawnWorldPosition;
              // delete currentSection.hasActivated;
            } else {
              // Interpolate towards final target
              currentSection.body.x += (finalTargetPathPosition.x - currentSection.body.x) * this.sectionSpawnSpeed;
              currentSection.body.y += (finalTargetPathPosition.y - currentSection.body.y) * this.sectionSpawnSpeed;
            }
          }
        }
      } else if (finalTargetPathPosition) {
        // Standard, non-spawning update
        currentSection.body.x = finalTargetPathPosition.x;
        currentSection.body.y = finalTargetPathPosition.y;
      } else if (!finalTargetPathPosition && i > 0) {
        // Fallback for non-spawning, if target is bad (e.g. path too short temporarily)
        // This section might become briefly invisible or stay at last known good spot.
        // currentSection.alpha = 0; // Example: hide it
      }
  ...
  onCycleComplete: function () {
    if (this.queuedSections > 0) {
      ...
      var newSec = this.addSectionAtPosition(lastSec.body.x, lastSec.body.y);
      // Mark the new section for spawn animation
      newSec.isSpawning = true;
      newSec.spawnWorldPosition = new Phaser.Point(newSec.body.x, newSec.body.y);
      newSec.hasActivated = false;
      this.queuedSections--;
    }
  },
  /**
