...
Snake.prototype = {
  ...
  update: function () {
    ...
    var distance = Phaser.Math.distance(headX, headY, worldCenterX, worldCenterY);

    if (distance > this.game.worldRadius - 10) {
      this.destroy();
      return; // Stop further updates if snake is destroyed
    }
  ...
  destroy: function () {
    ...
    var indexInGameSnakes = this.game.snakes.indexOf(this);
    if (indexInGameSnakes !== -1) {
      this.game.snakes.splice(indexInGameSnakes, 1);
    } else {
      // Fallback if not found, though it should be.
      // Consider if this indicates an issue elsewhere if it happens.
    }
    //remove constraints
  ...
  edgeContact: function (phaserBody) {
    ...
    if (!phaserBody || !phaserBody.sprite) {
      return; // No body or sprite to collide with
    }

    var otherSprite = phaserBody.sprite;
    var otherSnake = otherSprite.snake;

    // Case 1: My edge hits my own section (glitch avoidance or self-collision)
    if (otherSnake === this || this.sections.indexOf(otherSprite) !== -1) {
      this.edge.body.x = this.head.body.x;
      this.edge.body.y = this.head.body.y;
      return;
    }

    // Case 2: My edge hits another snake's part
    if (otherSnake) {
      // otherSnake is a valid snake instance and it's not me
      if (!otherSnake.isBeingDestroyed) {
        // The other snake is not already dying, so I (the attacker) die.
        this.destroy();
      }
      // If otherSnake.isBeingDestroyed is true, it means they were already
      // taken out (e.g., by hitting me or someone else just before this
      // specific contact was processed). So, I survive this particular interaction.
    }
    // If otherSprite had no .snake property, or otherSnake was undefined,
    // it implies a collision with something unexpected. Current logic does nothing.
  },
  /**
